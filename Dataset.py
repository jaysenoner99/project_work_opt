...
class Dataset:
    ...
    def hessian(self, weights,hess_trick=0):
        ...
        D = np.diag(d)
        return np.dot(np.dot(self.data.T, D), self.data) + 2 * lambda_reg * np.identity(weights.shape[0]) + hess_trick * 10**(-12) * np.identity(weights.shape[0])
    ...
    def test_solver(self, initial_weights, solver_to_call):
        ...
        print("Absolute error between the optimal value and solution:", np.abs(self.compute_log_loss(self.optimal_point) - self.compute_log_loss(solution)))
